{{ Form::model($paperCutting, ['route' => ['paper-cutting.update', $paperCutting->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data']) }}
<div class="modal-body">
    <div class="row">
        ...
        <div class="form-group col-md-6">
            ...
            @if(!empty($paperCutting->papercutting_file))
                <a href="{{ asset(Storage::url($paperCutting->papercutting_file)) }}" target="_blank" class="d-block mt-1">
                    {{ __('View Existing File') }}
                </a>
            @endif
        </div>
    </div>
...
<script>
    var baseUrl = "{{ route('paper-cutting.update', ':id') }}";
</script>
