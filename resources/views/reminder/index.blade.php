...
@section('card-action-btn')
    @if (Gate::check('create reminder'))
        <a class="btn btn-secondary btn-sm ml-20 customModal" href="#" data-size="lg"
            data-url="{{ route('reminder.create') }}" data-title="{{ __('Create Reminder') }}"> <i
                        ...
                        @if (Gate::check('create reminder'))
                            <div class="col-auto">
                                <a class="btn btn-secondary customModal" href="#" data-size="lg"
                                    data-url="{{ route('reminder.create') }}" data-title="{{ __('Create Reminder') }}">
                                    <i class="ti ti-circle-plus align-text-bottom"></i>{{ __('Create Reminder') }}</a>
                            </div>
                        @endif
                    </div>
                </div>
                                                ...
                                                <div class="cart-action">
                                                    ...
                                                    @if (Gate::check('show reminder'))
                                                        <a class="avtar avtar-xs btn-link-warning text-warning customModal" data-size="lg"
                                                            ...
                                                            data-url="{{ route('reminder.show', encrypt($reminder->id)) }}"
