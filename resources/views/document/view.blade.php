...
<body class="landing-page" data-pc-preset="{{ $settings['accent_color'] }}" data-pc-sidebar-theme="light"
    ...
    <div class="front-header-image">
        <div class="bg-img-overlay"
            style="background-image: url('{{ asset('assets/images/pages/header-bg.jpg') }}')"></div>
        <div class="privacy-details">

            <h1 class="text-center text-white mb-0 f-46">{{ $Document->name }}</h1>
            <p class="text-center text-white mb-0 f-16">{{ __('Last updated on') }}
                {{ dateFormat($today) }}</p>
            <p class="text-center text-white pt-3 f-16">{{ $Document->description }}</p>
        </div>
        <div class="container">
